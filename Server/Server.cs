...
namespace AppServer
{
    ...
    public class Server
    {
        ...
        public void BindDevice(string deviceName, string assemblyName)
        {
            ...
            //  Console.WriteLine(deviceName + " " + assemblyName); 
        }
        ...
        public List<DeviceInfo> QueryRemoteDeviceInfo(bool connectionStr = false)
        {
            ...
            if (connectionStr)
            {
                var connectionString = 
                    String.Format("mongodb+srv://{0}:{1}@cluster0-zragv.mongodb.net/test?retryWrites=true&w=majority", auth.User, auth.Pass);
                mongo = new MongoClient(connectionString);
            }
            else
            {
